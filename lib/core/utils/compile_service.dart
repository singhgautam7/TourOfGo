import 'dart:convert';
import 'package:http/http.dart' as http;
import '../models/tour_models.dart';

const String _fixedModules = '''
-- go.mod --
module example
require golang.org/x/tour v0.0.0-20201207214521-004403599411
-- go.sum --
golang.org/x/crypto v0.0.0-20190308221718-c2843e01d9a2/go.mod h1:djNgcEr1/C05ACkg1iLfiJU5Ep61QUkGW8qpdssI0+w=
golang.org/x/net v0.0.0-20190311183353-d8887717615a/go.mod h1:t9HGtf8HONx5eT2rtn7q6eTqICYqUVnKs3thJo3Qplg=
golang.org/x/sys v0.0.0-20190215142949-d0b11bdaac8a/go.mod h1:STP8DvDyc/dI5b8T5hshtkjS+E42TnysNCUPdjciGhY=
golang.org/x/text v0.3.0/go.mod h1:NqM8EUOU14njkJ3fqMW+pc6Ldnwhi/IjpwHt7yyuwOQ=
golang.org/x/tools v0.0.0-20190312164927-7b79afddac43/go.mod h1:LCzVGOaR6xXOjkQ3onu1FJEFr0SW1gC7cKk1uF8kGRs=
golang.org/x/tour v0.0.0-20201207214521-004403599411 h1:dJ4kVwSGlrLZXW6eo2IOer4Pm3wl2GIG4fytRziMgL8=
golang.org/x/tour v0.0.0-20201207214521-004403599411/go.mod h1:qMugOFWX59KzC8Nx7f2uvXxKxAqJfi1J6ZUHAWKnrRA=
''';

String buildCompileBody(String code) {
  final full = '$code\n$_fixedModules';
  return 'version=2&body=${Uri.encodeComponent(full)}';
}

class CompileService {
  static const _compileUrl = 'https://go.dev/_/compile?backend=';

  Future<CompileResponse> compile(String code) async {
    final body = buildCompileBody(code);
    final response = await http.post(
      Uri.parse(_compileUrl),
      headers: {
        'content-type': 'application/x-www-form-urlencoded',
        'x-requested-with': 'XMLHttpRequest',
        'origin': 'https://go.dev',
        'referer': 'https://go.dev/tour/',
      },
      body: body,
    );

    if (response.statusCode != 200) {
      throw Exception('Compile API returned ${response.statusCode}');
    }

    final json = jsonDecode(response.body) as Map<String, dynamic>;
    return CompileResponse.fromJson(json);
  }
}
